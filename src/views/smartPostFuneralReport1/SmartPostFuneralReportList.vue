<template>
  <a-card :bordered="false">
    ...
    <smart-post-funeral-report-modal
      ref="modalForm"
      @ok="modalFormOk"
...
export default {
  ...
  methods: {
    ...
    postEdit(preId) {
      console.log(preId)
      getAction('/smartPostFuneralReport/smartPostFuneralReport/queryByPreId', { id: preId, preId: preId }).then(
        (record) => {
          if (record.success) {
            //传到编辑
            console.log(record)
    ...
    postDelete(record) {
      ...
      getAction('/smartPostFuneralReport/smartPostFuneralReport/queryById', { id: record.id, preId: record.id }).then(
        (res) => {
          if (res.success) {
            //传到删除
            console.log(res)
