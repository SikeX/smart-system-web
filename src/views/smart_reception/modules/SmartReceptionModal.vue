...
<script>
  ...
  import {postAcion} from '../../test/invoice'
  ...
  export default {
    ...
    methods: {
      ...
      scanOk(url) {
        ...
        postAcion('/invoice/recognize', this.model[field]).then((res) => {
          if (res.success) {
            this.invoiceData = JSON.stringify(res.result.invoiceData.data.data.发票金额)
            this.$message.success('查询成功')
          } else {
            this.$message.error(res.message)
          }
        })
        putAction('/smart_reception/smartReception/updateCost',{id:record.id,cost:this.invoiceData}).then((res) => {
          if (res.success) {
            this.loadData()
          } else {
            this.$message.error(res.message)
