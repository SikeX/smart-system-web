              ...
              <article class="ml5 mr5 pt10">
                ...
                <section class="mt20">
                  <div class="lh-menu-wrap">
                    <menu id="lh-menu" class="lh-menu mt5 mr5">
                      <ul>
                        <!-- 课程章节目录 -->
                        <li v-for="chapter in chapterVideoList" :key="chapter.id" class="lh-menu-stair">
                          <a :title="chapter.title" href="javascript: void(0)" class="current-1">
                            <em class="lh-menu-i-1 icon14 mr10" />{{ chapter.title }}
                          </a>
                          <ol class="lh-menu-ol" style="display: block">
                            <li v-for="video in chapter.children" :key="video.id" class="lh-menu-second">
...
export default {
  ...
  methods: {
    handlePreview(video) {
      let url = window._CONFIG['ossFileUrl'] + '/' + video.wordOneUrl
    ...
    initCourseInfo() {
      learn.getAllChapterVideo(this.courseId).then((response) => {
        console.log(response)
