<template>
  ...
  >
    <a-spin tip="Loading..." :spinning="false">
      <a-input-search v-model="searchValue" style="margin-bottom: 1px" placeholder="请输入村镇(街道)名进行搜索" />
...
<script>
import { queryDepartTreeList, queryVillageAndStreetIdTree } from '@/api/api'
export default {
  name: 'SelectVillageStreetDepartModal',
  ...
  methods: {
    ...
    loadDepart() {
      // 获取村镇(街道)部门
      queryVillageAndStreetIdTree().then((res) => {
        console.log(res)
        if (res.success) {
          console.log(res.result)
          // let arr = [...res.result[0].children]
          let arr = res.result
          console.log(arr)
